...
(function($) {
    Drupal.ATTLocked = Drupal.ATTLocked || {};
    Drupal.ATTLocked.currentRegion = null;
    Drupal.ATTLocked.regions = [];
    Drupal.ATTLocked.clickScrolling = false;
    Drupal.ATTLocked.menuItems = [];
    var lang = $('html').attr('lang');
    Drupal.behaviors.scrollMenuAction = {
        attach: function(context) {
            if ($('body.front').length) {
                $('body.front .wrap').children().each(function() {
                    id = $(this).attr('id');
                    menu_item = $('#menu-main-menu li .' + id);
                    if (menu_item.length) {
                        Drupal.ATTLocked.menuItems.push(menu_item);
                        menu_item.attr('data-id', id);
                        menu_item.attr('data-href', menu_item.attr('href'));
                        menu_item.attr('data-link-type', 'scroll');
                        menu_item.attr('data-hash', Drupal.settings.basePath +lang+ "#" + id);
                        menu_item.attr('href', Drupal.settings.basePath +lang+ "#" + id);
                    }
                });
                for (x in Drupal.ATTLocked.menuItems) {
                    menu_item = Drupal.ATTLocked.menuItems[x];
                    $(menu_item).click(function() {
                        ...
                        if (data_type == 'scroll') {
                            ...
                            window.setTimeout(function() {
                                for (xx in Drupal.ATTLocked.menuItems) {
                                    m_item = Drupal.ATTLocked.menuItems[xx];
                ...
                $(window).load(function() {
                    for (x in Drupal.ATTLocked.menuItems) {
                        var menu_item = Drupal.ATTLocked.menuItems[x];
                    ...
                    $(window).scroll(function() {
                        ...
                        var current_delta = 0;
                        var min_delta = 10000;
                        for (i = 0; i < Drupal.ATTLocked.menuItems.length; i++) {
                            var menu_item = Drupal.ATTLocked.menuItems[i];
                        ...
                        if (selected_item !== Drupal.ATTLocked.currentRegion) {
                            for (j = 0; j < Drupal.ATTLocked.menuItems.length; j++) {
                                if (j !== selected_item) {
                                    $(Drupal.ATTLocked.menuItems[j]).removeClass('active');
                                }
                            }
                            $(Drupal.ATTLocked.menuItems[selected_item]).addClass('active');
                        }
                    });
                });
            }
            if ($('body.not-front').length) {
                $('body.not-front #menu-main-menu li a').each(function() {
                    data_link = $(this).attr('data-link');
                    if (data_link !== undefined) {
                        $(this).attr('href', Drupal.settings.basePath + "#" + $(this).attr('data-link'));
                    }
//                   
                });
            }
...
//        }
        ...
        $('.wrap .section').attr('id', function(i) {
        ...
        $('#block-system-main').on('click', '.schedule-control', function(e) {
            e.preventDefault();
            $e = $(this);
            var date_at = $e.attr('date-at');
            var step = $e.attr('step');
            var lang = $e.attr('lang');
            $.ajax({
                type: "get",
                url: '/'+lang+'/api/booking',
                dataType: "json",
                data: {date_at: date_at, nid: $('#hdd_nid').val(),step:step},
                beforeSend: function() {
                    $e.parents('div.schedule-container:first').find('.schedule-control').addClass('disabled');
                    $e.parents('div.schedule-container:first').find('.schedule-week').html('<i class="fa fa-circle-o-notch fa-spin"></i>');
                },
                complete: function() {
                },
                success: function(result) {
                    if (result.success) {
                        ...
                        $e.parents('div.schedule-container:first').find('.schedule-table').html(result.html);

                        if (result.hasOwnProperty("pagination")) {
                            if (result.pagination.prev != null)
                                $e.parents('div.schedule-container:first').find('.schedule-control.prev').attr('date-at', result.pagination.prev).attr('step', step).removeClass('disabled');
                            if (result.pagination.next != null)
                                $e.parents('div.schedule-container:first').find('.schedule-control.next').attr('date-at', result.pagination.next).attr('step', step).removeClass('disabled');
                        }
                        if (result.hasOwnProperty("schedule_week"))
                            $e.parents('div.schedule-container:first').find('.schedule-week').html(result.schedule_week);
                    }
                }
            });
...
var InfinitePage = {
    reset: function() {
        jQuery("#list-blog").data("page", 0).empty(), this.start()
    },
    start: function() {
        this.goodEnoughSinceAd = 0,
                jQuery("#list-blog").data("page") == undefined && jQuery("#list-blog").data("page", 0),
                jQuery("#list-blog").data("total-count") == undefined && jQuery("#list-blog").data("total-count", jQuery('#list-blog').attr('data-total-pages')),
                this.scrollHandler(),
                setTimeout(this.scrollHandler, 1500),
                jQuery(window).scroll(this.scrollHandler);
    },
    viewportDistanceFromBottom: function() {
        var a = jQuery(window).height();
        return jQuery(document).height() - (a + jQuery(window).scrollTop())
    },
    nearBottom: function() {
        return InfinitePage.viewportDistanceFromBottom() < 800;
    },
    checkScroll: function() {
        InfinitePage.scrollHandler();
    },
    scrollHandler: function() {
        if (InfinitePage.nearBottom()) {
            var a = jQuery("#list-blog").data("page") + 1;
            jQuery("#load_more_path").length == 0 || a > jQuery("#list-blog").data("total-count") ? InfinitePage.stop() : InfinitePage.loadingNextPage || (jQuery("#loading").show(), InfinitePage.loadingNextPage = !0, jQuery.ajax({
                url: jQuery("#load_more_path").val() + a,
                success: function(b) {
                    var c = b;
                    jQuery("#list-blog").append(c).data("page", a), jQuery("#loading").hide(), InfinitePage.loadingNextPage = !1;
                    _masonry();
                }
            }));
        }
    ...
    stop: function() {
        jQuery(window).unbind("scroll", InfinitePage.scrollHandler), jQuery("#loading").hide();
    }
};
function _masonry() {
    var container = document.querySelector('#list-blog');
